...
const topggClient = require("../index");
const schema = require("../Models/GuildDB")
...
module.exports.run = async (client, message) => {
  ...
  if (!command) return
    ...
    //Voice Only
    let hasVoted = await topggClient.hasVoted(message.author.id)
    if(command.voteOnly && !hasVoted){
   
      return message.reply({
      content: ` You are not a premium user, You need to vote me to access premium commands for free.`,
